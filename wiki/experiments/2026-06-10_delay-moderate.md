...
- **Commit hash:** `2c04ef76-dirty` (embedded in `delay.csv`; resolved from
  the uncommitted T46 working tree on top of `2c04ef7` `task 46: start`).
  Regenerate the dataset after the human commits for a clean provenance
  hash — the re-run command below reproduces it byte-identically.
