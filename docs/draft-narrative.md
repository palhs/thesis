...
|:--|:--|:--|:--|:--|
| RQ1 | commit-latency scaling as network-delay variance rises nominal→heavy-tailed | network timeline (Family B) | §4.3.1 | **closed** — stated as the delay/time-to-finality result. (2026-06-30, T77) The cross-chapter comparison tables (5.1, 6.1) and the radar now report the delay result as *relative finality slowdown* (× baseline: Casper FFG ×1.3 < PBFT ×1.9 < Snowman ×12–13) rather than mixed absolute/percent/multiplier units; §4.3.1 prose unchanged (already relative). (2026-07-01, coherence pass) §4.3.1 now states PBFT's and Casper FFG's relative slowdowns explicitly (PBFT ≈×1.9 at n=10 / ≈×2.0 at n=25, 1000→~1950 ms; Casper ×1.3) — the earlier "prose unchanged (already relative)" was inaccurate for PBFT, whose multiplier was absent, so Table 5.1/6.1's ×1.9 now traces upstream. Snowman corrected ×12–13 → ×12–15 across Table 5.1/6.1, §5.3, §4, and the abstract to match ch4's measured "twelve to fifteen" range (n=10 exponential = ×15.3); the dated T77 notes below retain their original ×12–13 wording as historical record. |
...
| RQ3 | relative communication overhead (msgs per agreed unit) | n (Family A) | §4.2 (was §4.2.4 pre-Wave-4) | **closed** — "answers RQ3" stated. (2026-06-30, Wave-4) §4.2 was de-subsectioned in the examinability cut, so the RQ3 home is now §4.2 (the overhead paragraph + Fig 4.1, formerly Fig 4.2). (2026-07-01, coherence pass) RQ3 rescoped to **message count only**: the byte-per-unit measurand was retired because the harness's `bytes_per_acu` is payload-dominated at the thesis workload (it tracks transaction payload amortized over committed units, not protocol message structure — TASKS.md Backlog 2026-06-14, T49 validation), so the O(n²)-vs-O(K·β) overhead contrast RQ3 tests is legible only in message count. ch1 §1.5 + ch3 Table 3.3 reworded to promise messages per agreed unit (the `bytes_per_acu` schema row dropped); ch4 §4.2 (already messages-only) is now complete against the narrowed question. |
...
- **Surface the inversions.** The thesis's non-obvious results are the
  ...
  - the same subsampling keeps Snowman live (finalizing, far slower — it pays the
    steepest finality slowdown under delay, ×12–15) when peers are merely slow, yet
...
- **Latency-only model:** the no-compute/no-bandwidth-cost caveat attaches to
  ...
  **not** qualify the message-count, liveness, or safety verdicts.
