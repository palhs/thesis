...
[[wiki/sources/2026-04-21_buterin-gasper-2020]] — not the specific fivefold gap,
which moves with the calibration.
...
so neither separates the protocols — that comes from the delay and adversarial
axes. Communication overhead already separates them by an order of magnitude in a
...
— so enough retries eventually succeed even under heavy loss, the recovery visible
in the view-change count climbing with loss into the tens at `n = 10` and toward
the seventies at `n = 25` (Figure 4.5b).
...
slope — an argument that predicts the cliff's location and the committee ordering,
used qualitatively [[wiki/experiments/2026-06-13_delay-analysis]].
...
the worst loss they survive, converting that cost into survival — PBFT a long tail
to 20% loss, Snowman a high but brittle plateau
...
under the curve, retaining 0.90 finality at 5% loss, PBFT on survival depth, alone
alive at 20% [[wiki/experiments/2026-06-13_delay-comparison]]. Casper FFG's
...
ordering — PBFT most robust, Casper FFG most fragile — is a property of the
...
differ entirely above it — in the kind of failure, not its onset. Liveness too
holds below the threshold with one exception (Figure 4.7a): Casper FFG's success
...
proposer cadence and window length, not the validator-set size), and it is
...
The safety order is therefore Snowman, Casper FFG, PBFT — about what failure occurs
above `φ = 0.33`, not which fraction each tolerates. Two cautions: the three
...
independently of any protocol — and the joint-regime synthesis are taken up in
Chapter 5.
