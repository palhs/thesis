...
At `n = 10`, the three protocols separate clearly. PBFT's finalization rate declines gradually and remains above zero even at 20% loss. At the other extreme, Casper FFG collapses at the first loss step, down to near zero by 5%. Snowman falls between the two, holding partially at 5% loss before it cliffs to near zero by 10%. Ranked by AURC with survival depth as the tiebreak, the order is strict: PBFT, Snowman, Casper FFG.

At `n = 25` the picture changes for Snowman. Its finalization rate stays high through 5% loss, near the PBFT level, then falls to near zero at 10%, a clear plateau-then-cliff shape absent at `n = 10`. PBFT keeps its shallow tail to 20% loss, and Casper FFG collapses as before. PBFT and Snowman tie at the top by AURC while Casper FFG remains last.
...
The two stress axes converge on one result: the protocols that survive loss pay for it in latency. PBFT and Snowman both inflate their time-to-finality by factors of roughly two to three-and-a-half at the worst loss they survive; Casper FFG does not make the trade: it inflates latency by only three to ten percent over the few seeds that still finalize, and gains nothing for it. No configuration is both cheap and resilient: protecting liveness against a lossy network costs latency, and the only variable is how much.

PBFT degrades most gracefully, alive at the deepest tested loss at both committee sizes; Snowman is strong but brittle, best at light loss with a large committee but prone to sudden collapse; Casper FFG is fragile and never establishes a resilient plateau. The `n = 25` PBFT–Snowman result is a non-rejection: the two AURC values sit inside overlapping intervals.

## 4.4 Adversarial sweep

The adversarial sweep holds the network at a constant baseline delay and replaces a fraction `φ` of the honest validators with a single Byzantine strategy, isolating adversarial behavior from the network effects of §4.3. It draws run family C and exercises three generic capabilities: delayed voting, silent non-participation, and equivocation (defined in §3.4). Each is swept from an honest control through a band of fractions `φ` at `n ∈ {10, 25}`, twenty seeds per cell; `φ` is denominated in each protocol's natural unit (replicas for PBFT and Snowman, stake for Casper FFG). The sweep extends past the one-third threshold where a safety failure is possible. It reports two outcome families: liveness (success rate, §3.5) and safety, a per-protocol safety-violation rate. The exception is Snowman, whose probabilistic finality is reported through its analytical bound `ε` rather than a fork count.

### 4.4.1 Delayed voting

Under delayed voting the three protocols separate by failure mode, driven by protocol structure rather than the size of the delayed set (Figure 4.4a,b). PBFT is immune: its delayed validators are backups, the honest remainder still meets the `2f+1` quorum, so the success rate holds at 1.0. Casper FFG loses liveness instead, because its single rotating proposer is periodically the delayed node and the block it owes stalls for that slot, which drops the success rate to a worst pooled 0.60–0.65. Snowman keeps liveness, but its time-to-finality explodes by a factor of roughly 62 at `n = 10` and 49 at `n = 25`, severe near `φ = 0.20`, because each of its `β` sequential poll rounds waits on the slowest sampled peer. None of the three forks: delayed voting threatens liveness and latency but not agreement.
...
quorum cliff: it finalizes with no goodput loss up to `φ = 0.33` and dies at
